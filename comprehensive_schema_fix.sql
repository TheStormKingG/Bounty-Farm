-- COMPREHENSIVE FIX: Remove all payment_status references and update schema
...
-- 2) Drop ALL existing triggers on sales_dispatch table to prevent conflicts
DROP TRIGGER IF EXISTS trigger_create_invoice_and_dispatch ON sales_dispatch;
DROP TRIGGER IF EXISTS trigger_create_invoice ON sales_dispatch;
DROP TRIGGER IF EXISTS trigger_create_dispatch ON sales_dispatch;
DROP TRIGGER IF EXISTS create_invoice_on_po_insert ON sales_dispatch;
DROP TRIGGER IF EXISTS create_dispatch_on_invoice_paid ON invoices;

-- 3) Drop ALL existing trigger functions that might reference payment_status
DROP FUNCTION IF EXISTS public.create_invoice_and_dispatch_on_po();
DROP FUNCTION IF EXISTS public.create_invoice_on_po_insert();
DROP FUNCTION IF EXISTS public.create_dispatch_on_invoice_paid();
DROP FUNCTION IF EXISTS public.create_invoice_trigger();

-- 4) Remove payment_status column from invoices table (if it exists)
ALTER TABLE public.invoices DROP COLUMN IF EXISTS payment_status;

-- 5) Add payment_status column to dispatches table (if it doesn't exist)
ALTER TABLE public.dispatches ADD COLUMN IF NOT EXISTS payment_status character varying DEFAULT 'pending';

-- 6) Create NEW trigger function for automatic invoice and dispatch creation
...
-- 7) Create NEW trigger on sales_dispatch table
